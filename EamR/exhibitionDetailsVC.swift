...
class exhibitionDetailsVC: UIViewController, UITableViewDelegate, UITableViewDataSource, FloatRatingViewDelegate, MXSegmentedPagerDelegate, MXSegmentedPagerDataSource {
    ...
    override func viewDidLoad() {
        ...
        print("Selected Exhibition: \(self.selectedExhibition)")
    ...
    func getExhibitorsList() {
        ...
        manager.post((url!.absoluteString), parameters: param, progress: nil, success: {(operation: URLSessionTask, responseObject: Any) -> Void in
            self.appDelegate.showProgress(false)
            print("Exhibitor Response Success!!")
            
            let responseDictionary = (responseObject as! NSDictionary)
            print("Exhibitor Response: \(responseDictionary as Any)")
            ...
            if (responseDictionary["status"] as AnyObject).integerValue == 1 {
                ...
                if (self.exhibitorsListArray.count != 0){
                    ...
                    if (self.isSearchExhib == true) {
                        ...
                        let alertController = UIAlertController(title: "Sorry", message:"No Exhibitor Found", preferredStyle: .alert)
                        let ok = UIAlertAction(title: "OK", style: .default, handler: {(_ action: UIAlertAction) -> Void in
                            
                            self.isSearch = true
                            self.searchBar.text = ""
                            self.getExhibitorsList()
    ...
    func updatedSearchedExhibitor() {
        ...
        if UserDefaults.standard.bool(forKey: "loggedIn") == true {
            ...
            let param: [String: Any] = ["search_name" : searchBar.text! , "product_price":"0.0",  "user_id":UserDefaults.standard.value(forKey: "ProfileID")!, "user_name":userName, "user_email":userEmail, "exhibitor_srch_perm" : "Y"]
    ...
    func getExhibitionReviews() {
        ...
        print("Exhibition Review list: \(param)")
    ...
    func tableView(_ tableView: UITableView, cellForRowAt indexPath: IndexPath) -> UITableViewCell {
        if tableView == exhibitorsListTableView {
            ...
            cell?.countryLbl?.text = dict["exhibior_country"] as? String
            cell?.venuLbl?.text = dict["name"] as? String
    ...
    func loadExhibitionData() {
        ...
        print("Exhib explanation: \(self.selectedExhibition["explanation"] as? String)")
